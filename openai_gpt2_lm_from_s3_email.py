...
def main():
    ...
    parser.add_argument("--s3_key", type=str,
                        default="e2e_training/email_data.pkl")
    ...
    def pre_process_dataset(encoded_dataset, input_length):
        ...
        for i, tokens in enumerate(encoded_dataset):
            ...
            try:
                response_index = tokens.index(26209) + 2
                lm_labels[i,  response_index: len(tokens)-1] = tokens[response_index+1 : input_length] 
            except:
                lm_labels[i, :len(tokens)-1] = tokens[1:input_length]
