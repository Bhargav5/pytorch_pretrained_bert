...
def main():
    ...
    special_tokens = ['<INF>', '<FOR>','<CON_START>','<START>','<END>']
    ...
    def pre_process_dataset(encoded_dataset, input_length, start_token_id):
        ...
        input_sen_len = input_length // 2

        for i, tokens in enumerate(tqdm(encoded_dataset)):
            try:
                start_id_index = tokens.index(start_token_id)
                input_ids[i, :start_id_index + 1] = tokens[:start_id_index + 1]
                #input_ids[i, :start_id_index] = tokens[:start_id_index]
                #input_ids[i, input_sen_len:input_sen_len + len(tokens)- start_id_index] = tokens[start_id_index :]
            
                #lm_labels[i, input_sen_len:input_sen_len + len(tokens)- start_id_index - 1] = tokens[start_id_index + 1:]
                lm_labels[i, start_id_index:len(tokens)-1] = tokens[start_id_index + 1:]
